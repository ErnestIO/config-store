...
ADD . /go/src/github.com/${GITHUB_ORG:-ernestio}/config-store
WORKDIR /go/src/github.com/${GITHUB_ORG:-ernestio}/config-store
...
COPY --from=compiler /go/bin/config-store .
